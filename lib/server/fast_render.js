...
export const FastRender = {
  ...
  async onPageLoadWithoutSink(callback) {
    Meteor.startup(() => {
      WebAppInternals.registerBoilerplateDataCallback(
        'meteor/server-render',
        async (req, _data, _arch, res) => {
          // After the boilerplate data callbacks run, webapp unconditionally calls
          // res.writeHead() (see webapp_server.js); disableBoilerplateResponse() only skips
          // piping the boilerplate body, not this header write. In the "without sink" flow the
          // callback fully owns and ends the response itself, so guard writeHead to avoid a
          // spurious ERR_HTTP_HEADERS_SENT once the response has already been sent.
          if (res && !res._frWriteHeadGuarded) {
            res._frWriteHeadGuarded = true;
          ...
          const frContext = await new FastRender._Context(
            req.cookies.meteor_login_token,
            {
              headers: req.headers,
            },
          ).init();
          ...
          await FastRender.frContext.withValue(frContext, async function() {
            await callback(req, _data, _arch, res);
